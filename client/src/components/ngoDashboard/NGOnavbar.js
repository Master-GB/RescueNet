...
const NGOnavbar = ({ ngoData, handleStatusToggle }) => {
  ...
  return (
    <nav className="bg-[#151316] text-[#e7e1e5] shadow-ambient px-8 py-4 flex items-center justify-between z-40 sticky top-0 border-b border-[#201e23]">
      <div className="flex items-center space-x-8 text-sm">
        <div className="flex items-center font-bold text-xl text-[#e7e1e5] tracking-tight mr-4">
        ...
        <div className="space-x-8 lg:flex hidden">
          ...
          <NavLink 
            to="/ngo/campaigns" 
            className={({isActive}) => isActive ? 'border-b-2 border-primary text-primary pb-1 font-semibold' : 'text-gray-400 hover:text-primary transition-colors pb-1'}
          >
            Donation campaigns
          </NavLink>
        </div>
