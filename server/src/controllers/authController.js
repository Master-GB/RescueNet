...
export const login = async (req, res) => {
  ...
  return res
  .status(200)
  .json({ success:true, message: "Logged in successfully", user: { id: user._id, name: user.name, email: user.email, role: user.role },
  });
  }catch(error){
    return res
      .status(500)
...
export const me = async (req, res) => {
  res.json({ user: req.user }); // req.user set by auth middleware
};
